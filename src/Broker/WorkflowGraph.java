...
public class WorkflowGraph {
	...
	private final int standardMIPS = 100;
	private final int standardDataTransfer = 1;
	...
	public boolean convertDagToWorkflowGraph(Adag dag) {
		...
		for (Job job : dag.getJob()) {
			...
			for (FilenameType file : job.getUses()) {
				long filesize= Long.valueOf(file.getSize())*standardDataTransfer;
				...
				if (file.getLink() == LinkageType.INPUT && Long.valueOf(file.getSize()) > 0)
					inputFilesSize += Long.valueOf(file.getSize())*standardDataTransfer;
				else if (file.getLink() == LinkageType.OUTPUT && Long.valueOf(file.getSize()) > 0)
					outputFilesSize += Long.valueOf(file.getSize())*standardDataTransfer;
			}
			...
			// temporary
			wfNode.setInstructionSize(runTime * standardMIPS);
							...
							if (outFile.getLink() == LinkageType.OUTPUT) {
								for (FilenameType inFile : childJob.getUses()) {
									if (inFile.getLink() == LinkageType.INPUT
										...
										double curSize = Long.valueOf(inFile.getSize())*standardDataTransfer;
										if (curSize > 0)
											size += curSize;
									}
								}
