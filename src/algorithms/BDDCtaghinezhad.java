...
public class BDDCtaghinezhad extends WorkflowPolicy {
	...
	public BDDCtaghinezhad(WorkflowGraph g, ResourceSet rs, long bw) {
		super(g, rs, bw);
	...
	public float schedule(int startTime, int deadline, float cost) {
		...
		System.out.print("rdis:" + redistributionCount);
		return (cost);
	}
	...
	protected void RecomputeECT_SubBudget() {

		// complexity Analysis: N+N:Nodes O(N)
		Map<String, WorkflowNode> nodes = graph.getNodes();
		int interval = resources.getInterval();
		float alphaBudget = remaingCost;
		// float alphaBudget= TotalCost*interval;
		float alpha = 0;
		long sum = 0;
		for (int i = 0; i < lvlList.size(); i++) {
	...
	private float CalculateCTTF(WorkflowNode curNode) {
		...
		float Time;
		long subDl;
		double Cost, subBudget, CTTF, BestCTTF = Integer.MAX_VALUE*-1;
		...
		subBudget = getLevelNode(curNode.getLevelBottem()).getSubBudget();
		...
		for (int curInst = 0; curInst < instances.getSize(); curInst++) {
			...
			if (CTTF > BestCTTF && r.cost < subBudget) {
				BestCTTF = CTTF;
				bestInst = curInst;
		...
		if (bestInst == -1) {
			...
			// if
			// (getLevelNode(getLevelNode(curNode.getLevelBottem()).getLevelid()).getSubBudget()>0)
			// {
			// getLevelNode(curNode.getLevelBottem()).setSubBudget(getLevelNode(getLevelNode(curNode.getLevelBottem()).getLevelid()).getSubBudget());
			// getLevelNode(getLevelNode(curNode.getLevelBottem()).getLevelid()).setSubBudget(0);
			// }
			// CalculateCTTF(curNode);
			// use get consumed cost;

			// use idle times if not goes behind deadline;
			if (bestInst == -1) {
