...
public class MyBDHEFT extends WorkflowPolicy {
	boolean backCheck = true;
	float factorTime=(float)0.3;
	float factorBudget=(float)0.7;
	// this is updated BDHEFT to cloud system. it orginal one use grid system
	// mechanism.
	public MyBDHEFT(WorkflowGraph g, ResourceSet rs, long bw) {
		super(g, rs, bw);
	...
	private float calcSWB(float cost) {
		...
		c = (float) (cost - TotalCost - selectCost);
		return c;
	}
	...
	private float calcSWD(float deadline) {
		...
		c = deadline -selectTime - NotselectTime;
		return c;
	}
		...
		while (!queue.isEmpty()) {
			...
			for (int curInst = 0; curInst < instances.getSize(); curInst++) {
				...
				if (r.finishTime <= CTD && r.cost <= CTB) {
					...
					balance = (float) (factorTime * normFinish + (factorBudget * normCost));
			...
			for (int curRes = resources.getSize() - 1; curRes >= 0; curRes--) { // because the cheapest one is the last
				...
				if (r.finishTime <= CTD && r.cost <= CTB) {
					//we should use normaliziation between cost and finishTime to get a good results
					float normFinish,normCost;
					normFinish= (r.finishTime- 1) / (deadline-1);
					normCost=(r.cost- 0) /(cost-0);
					balance = (float) (factorTime * normFinish + (factorBudget * normCost));
...
//			finishTime += Math.round((float) curNode.getInstructionSize() / resources.getResource(curRes).getMIPS());
//		}
